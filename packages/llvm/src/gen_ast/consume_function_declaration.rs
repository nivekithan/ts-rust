...
use crate::{build_expression::build_expression, to_basic_type::ToBasicType};
...
pub(crate) fn consume_function_declaration<'a>(
    ...
    let llvm_return_type = return_type.to_llvm_type(context);
    let param_types: Vec<BasicTypeEnum> = arguments
        .iter()
        .map(|(_, data_type)| {
            number_of_arguments += 1;
            return data_type.to_llvm_type(context);
        })
        .collect();
     ...
     * 
     * TODO: Find a way to change the name of the argument
     * 
     * */
    ...
    /*
     *
     * TODO: Add some way for adding the arguments into the symbol_table so that it can
     * be used by the code
     * 
     * */
